...
var banner = ['/*!\n',
    ' * Andy Tan Parallax - <%= pkg.title %> v<%= pkg.version %> (<%= pkg.homepage %>)\n',
...
gulp.task('less', function() {
    return gulp.src('assets/less/agency.less')
        .pipe(less())
...
gulp.task('minify-css', ['less'], function() {
    return gulp.src('assets/css/agency.css')
...
gulp.task('minify-js', function() {
    return gulp.src('assets/js/agency.js')
        .pipe(uglify())
        .pipe(header(banner, { pkg: pkg }))
        .pipe(rename({ suffix: '.min' }))
